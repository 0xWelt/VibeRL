...
class SnakeGameEnv(gym.Env):
    ...
    def _render_frame(self):
        ...
        for i, segment in enumerate(self.snake):
            color = (0, 200, 0) if i < len(self.snake) - 1 else (0, 255, 0)  # Green body, brighter head
            pygame.draw.rect(
                canvas, color,
                (segment[1] * self.cell_size + 1, segment[0] * self.cell_size + 1,
        ...
        if self.food:
            pygame.draw.rect(
                canvas, (255, 0, 0),  # Red
                (self.food[1] * self.cell_size + 1, self.food[0] * self.cell_size + 1,
                 self.cell_size - 2, self.cell_size - 2)
            )
